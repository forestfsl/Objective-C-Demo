...
@interface SLQRCode () <AVCaptureMetadataOutputObjectsDelegate>
...
@implementation SLQRCode

///构建方法
+ (instancetype)shareInstance
{
    static SLQRCode *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[SLQRCode alloc]init];
    });
    return instance;
}

+(UIImage *)buildQRCodeWithMessage:(NSString *)message codeType:(SLCodeType)type size:(CGSize)size {
    SLQRCode *instance = [SLQRCode shareInstance];
    instance.QRfilter = [CIFilter filterWithName:type == SLCodeTypeQRCode ? @"CIQRCodeGenerator" : @"CICode128BarcodeGenerator"];
    ///恢复默认
    [instance.QRfilter setDefaults];
    
    NSData *data;
    if (type == SLCodeTypeQRCode) {
        //纠错水平  L M Q H
...
+(void)scanWithVC:(UIViewController *)vc scanView:(UIView *)view lineView:(UIView *)lineView completeBlock:(void (^)(NSString *))completeBlock {
    SLQRCode *instance = [SLQRCode shareInstance];
