...
@interface SLFeedbackVC ()

@property(nonatomic,strong)SLFeedBackViewModel *viewModel;
...
@property(nonatomic,strong)SLDropView          *menuView;
@end
...
- (void)resetRightNavi
{
    ...
    [rightBtn setTitleColor:SLCOLOR(70, 70, 70, 1) forState:UIControlStateNormal];
    rightBtn.titleLabel.font            = [UIFont sl_NormalFont:15];
...
- (void)initView
{
    ...
    label.text              = @"意见反馈";
    label.font              = [UIFont sl_NormalFont:16];
    ...
    _ideaLabel.textColor    = SLCOLOR(100, 100, 100, 1);
    _ideaLabel.text         = @"功能意见";
    _ideaLabel.font         = [UIFont sl_NormalFont:14];
    ...
    tip.textColor           = SLCOLOR(80, 80, 80, 1);
    tip.text                = @"小贴士";
    tip.font                = [UIFont sl_NormalFont:14];
    ...
    tipLabel.textColor      = SLCOLOR(80, 80, 80, 1);
    tipLabel.text           = @"您也可以反馈意见至QQ：81520140";
    tipLabel.font           = [UIFont sl_NormalFont:14];
    [tipLabel setText:@"您也可以反馈意见至QQ:81520140" Font:[UIFont sl_NormalFont:14] withColor:[UIColor blueColor] Range:NSMakeRange(12, 8)];
...
- (UITextView *)textView
{
    if (!_textView)
    {
        ...
        _textView.font = [UIFont sl_NormalFont:16];
...
- (SLDropView *)menuView
{
    if (!_menuView)
    {
        _menuView = [[SLDropView alloc]initWithFrame:CGRectMake(0, 134, kWidth, 255) withTitleArray:@[@"功能意见",@"界面意见",@"您的新需求",@"操作意见",@"流量问题",@"其他"]];
        _menuView.backgroundColor = SLCOLOR(200, 200, 200, 0.7);
    }
    return _menuView;
}
