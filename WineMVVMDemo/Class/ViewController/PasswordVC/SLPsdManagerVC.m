...
#import "SLPsdManagerVC.h"
#import "SLPsdManagerViewModel.h"

@interface SLPsdManagerVC ()<UITextFieldDelegate>
@property(nonatomic,strong)SLPsdManagerViewModel   *viewModel;
/**原密码*/
...
- (void)bindViewModel
{
    [super bindViewModel];
    @weakify(self);
    RAC(self.viewModel,sl_originPsd)         = self.w_originTextField.rac_textSignal;
    RAC(self.viewModel,sl_newPsd)            = self.w_newTextField.rac_textSignal;
    RAC(self.viewModel,sl_confirmPsd)        = self.w_confirmTextField.rac_textSignal;
...
- (void)initView
{
    ...
    self.confirmBtn.titleLabel.font         = [UIFont sl_NormalFont:18];
    ...
    [self.confirmBtn setBackgroundImage:[UIImage imageFromColor:SLCOLOR(210, 210, 210, 1)] forState:UIControlStateDisabled];
...
- (UITextField *)w_originTextField
{
    if (!_w_originTextField)
    {
        ...
        view.backgroundColor        = SLCOLOR(252, 252, 252, 1);
        ...
        _w_originTextField.font             = [UIFont sl_NormalFont:16];
...
- (UITextField *)w_newTextField
{
    if (!_w_newTextField)
    {
        ...
        _w_newTextField.font                = [UIFont sl_NormalFont:16];
...
- (UITextField *)w_confirmTextField
{
    if (!_w_confirmTextField)
    {
        ...
        _w_confirmTextField.font            = [UIFont sl_NormalFont:16];
