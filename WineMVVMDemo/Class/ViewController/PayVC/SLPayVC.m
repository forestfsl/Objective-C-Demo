...
@interface SLPayVC ()<UITableViewDelegate,UITableViewDataSource>

@property(nonatomic,strong)SLPayViewModel  *viewModel;
...
- (void)initView
{
    ...
    self.priceLabel.font            = [UIFont sl_NormalFont:16];
    self.priceLabel.textColor       = [UIColor whiteColor];
    self.priceLabel.text            = [NSString stringWithFormat:@"实付款 ：¥%.2f",self.viewModel.price];
    
    
    self.payBtn.titleLabel.font     = [UIFont sl_NormalFont:18];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0)
    {
        ...
        if (!cell)
        {
            ...
            cell.textLabel.textColor        = SLCOLOR(150, 150, 150, 1);
            UIView *line                    = [[UIView alloc]initWithFrame:CGRectMake(12, 43.6, kWidth - 12, 0.4)];
            line.backgroundColor            = SLCOLOR(220, 220, 220, 1);
    ...
    else if (indexPath.section == 1)
    {
        ...
        {
            SLOrderDetailTableViewCell *cell   = [tableView dequeueReusableCellWithIdentifier:@"CELL2"];
            if (!cell)
            {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    header.backgroundColor = SLCOLOR(240, 240, 240, 1);
    return header;
}
