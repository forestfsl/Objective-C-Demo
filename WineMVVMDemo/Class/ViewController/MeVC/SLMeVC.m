...
@interface SLMeVC ()<UITableViewDelegate,UITableViewDataSource>
...
@property(nonatomic,strong)SLMeHeaderView      *headView;
/**设置按钮*/
...
- (void)configSelf {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"SLMeTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"cell"];
...
- (void)updateNavi:(id )x
{
    ...
    if (y >= 0)
    {
        ...
        NSLog(@"%f",a);
        self.navigationController.navigationBar.titleTextAttributes = @{NSForegroundColorAttributeName:SLCOLOR(70, 70, 70, a)};
        if (a < 0.9 && a >= 0)
        {
            NSLog(@"11111");
    ...
    else
    {
        ...
        if (!footView)
        {
            ...
            [titleArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                SLMeBtn *btn = [SLMeBtn buttonWithTitle:obj imageName:imgArray[idx]];
                btn.tag     = idx;
...
{
    SLMeTableViewCell *cell        = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    
    if (indexPath.section == 0)
    {
        [cell configCellTitle:@"我的订单" subTitle:@"查看全部订单"];
//        [cell updateTitle:@"我的订单" subTitle:@"查看全部订单"];
    }
    else if (indexPath.section == 1)
    {
        [cell configCellTitle:@"我的钱包" subTitle:@"查看明细"];
    }
    else
    {
        [cell configCellTitle:self.titleArray[indexPath.row] subTitle:@""];
        if (indexPath.row == 2)
        {
            [cell configCellTitle:self.titleArray[indexPath.row] subTitle:@"QQ81520140"];
        }
    }
...
- (UIView *)headView
{
    if (!_headView)
    {
        _headView = [[SLMeHeaderView alloc]initWithFrame:CGRectMake(0, 0, kWidth, 200)];
