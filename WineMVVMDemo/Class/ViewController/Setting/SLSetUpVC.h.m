...
@interface SLSetUpVC ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)SLSetupViewModel    *viewModel;
...
- (void)initView
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"SLSetupTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"cell"];
...
{
    SLSetupTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    NSArray *array  = @[[SLTool getCacheSize],@"",[SLTool getVersion],@"",@""];
    [cell configCellTitle:self.array[indexPath.row] subTitle:array[indexPath.row]];
    return cell;
}
...
- (UIButton *)exitBtn
{
    if (!_exitBtn)
    {
        ...
        _exitBtn.backgroundColor        = SLCOLOR(253, 253, 253, 1);
    }
    return _exitBtn;
}
