...
#import "SLOrderStateTableView.h"
...
@interface SLOrderDetailVC ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)SLOrderDetailViewModel  *viewModel;

@property(nonatomic,strong)UITableView              *tableView;

@property(nonatomic,strong)UISegmentedControl       *segment;

@property(nonatomic,strong)SLOrderStateTableView   *statusTable;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SLOrderDetailTableViewCell *cell = nil;
    if (indexPath.section == 0)
    {
        cell = [tableView dequeueReusableCellWithIdentifier:cellID0];
        if (!cell)
        {
            NSArray *array = [[NSBundle mainBundle]loadNibNamed:@"SLOrderDetailTableViewCell" owner:nil options:nil];
            cell = array[0];
        }
...
- (SLOrderStateTableView *)statusTable
{
    if (!_statusTable)
    {
        _statusTable = [[SLOrderStateTableView alloc]initWithFrame:CGRectMake(0, 64, kWidth, kHeight) style:UITableViewStylePlain];
...
- (UIView *)bottomView
{
    if (!_bottomView)
    {
        ...
        _bottomView.backgroundColor = SLCOLOR(30, 30, 30, 0.8);
...
- (UIButton *)doneBtn
{
    if (!_doneBtn)
    {
        ...
        _doneBtn.titleLabel.font = [UIFont sl_NormalFont:19];
