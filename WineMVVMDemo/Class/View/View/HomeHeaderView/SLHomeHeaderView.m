...
#import "SLHomeBannerCollectionViewCell.h"

@interface SLHomeHeaderView ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation SLHomeHeaderView
...
- (void)configSubView {
    ...
    [_collectionView registerNib:[UINib nibWithNibName:NSStringFromClass([SLHomeBannerCollectionViewCell class]) bundle:nil] forCellWithReuseIdentifier:@"cell"];
    ...
    for (int i = 0; i < 4; i++) {
        ...
        label.frame = CGRectMake(button.frame.origin.x - 2.5, y + 45 + 3, button.frame.size.width + 10, 20);
        label.font = [UIFont sl_NormalFont:12];
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((1.9 * NSEC_PER_SEC) * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        _isDragging = NO;
    });
}
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    SLHomeBannerCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://www.jiuyunda.net:90%@",self.imageArray[indexPath.row][@"img"]]];
    [cell.imageV sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"headPlaceholder.jpg"]];
    return cell;
}
