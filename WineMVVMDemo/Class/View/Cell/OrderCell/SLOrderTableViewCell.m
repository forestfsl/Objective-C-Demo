...
@interface SLOrderTableViewCell ()

@property(nonatomic,strong)NSMutableArray *imageArray;
@end

@implementation SLOrderTableViewCell
...
-(void)configCellData:(id)obj {
    _order = (SLOrderModel*)obj;
    self.w_timeLabel.text       = _order.created_at;
    self.w_statusButton.hidden  = YES;
    self.w_finishImage.hidden   = YES;
    if ([_order.workflow_state isEqualToString:@"generation"])
    {
        //        待付款
    ...
    else if ([_order.workflow_state isEqualToString:@"cancelled"])
    {
        //        已取消
        self.w_statusLabel.text                 = @"已取消";
    }
    else if ([_order.workflow_state isEqualToString:@"paid"])
    {
        //        待接单
        self.w_statusLabel.text                 = @"待接单";
    }
    else if ([_order.workflow_state isEqualToString:@"distribution"] || [_order.workflow_state isEqualToString:@"take"])
    {
        //        配送中
        self.w_statusLabel.text                 = @"配送中";
    }
    else if ([_order.workflow_state isEqualToString:@"completed"])
    {
        //        已完成
        self.w_statusLabel.text                 = @"已完成";
        self.w_finishImage.hidden               = NO;
    }
    else if ([_order.workflow_state isEqualToString:@"receive"])
    {
        //        已配送
    ...
    //    图片
    [self setGoodImage];
    self.w_priceLabel.text                      = [NSString stringWithFormat:@"%.2f",_order.paycost];
}
...
- (void)setGoodImage
{
    ...
    if (array.count == 1)
    {
        SLOrderDetailModel *model              = array[0];
        if (self.imageArray.count <1)
        {
            ...
            imageView.layer.borderColor             = SLCOLOR(200, 200, 200, 1).CGColor;
        ...
        if (![_w_scrollView viewWithTag:100])
        {
            ...
            goodTitle.textColor                     = SLCOLOR(70, 70, 70, 1.0);
    ...
    else
    {
        ...
        for (int i = 0; i < array.count; i++)
        {
            ...
            SLOrderDetailModel *model          = array[i];
